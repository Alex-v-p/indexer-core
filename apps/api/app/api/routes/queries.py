...
from app.adapters.database.models import QueryRun
...
async def read_query_run(
    ...
    """Read a persisted query run with evidence, citations, and trace."""

    query_run = await get_query_run(session=session, query_run_id=query_run_id)
    if query_run is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Query run not found.")
    return to_query_response(query_run)


def to_query_response(query_run: QueryRun) -> QueryResponse:
