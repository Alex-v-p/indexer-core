...
CONTEXTUAL_RAG_CONFIG = PipelineConfig(
    ...
    description=(
        "Hybrid retrieval over neighborhood-contextualized chunk representations while returning original source text "
        "for answer generation and citations."
    ),
    tool_names=(
...
def build_contextual_rag_graph(*, retriever: Retriever, llm_provider: LLMProvider) -> GraphRunner:
    """Build the contextual graph: contextual hybrid retrieve → generate answer."""
